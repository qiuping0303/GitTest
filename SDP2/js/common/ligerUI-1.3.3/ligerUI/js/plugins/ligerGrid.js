...
//  };
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage:  "表示中の件数： {from} ～ {to} （全：{total} 件）",
        pageTextMessage: 'Page',
        loadingMessage: 'ロードしています...',
        findTextMessage: '查找',
        noRecordMessage: 'データが見つかりました',
        isContinueByDataChanged: '数据已经改变,如果继续将丢失数据,是否继续?',
        cancelMessage: '取消',
        saveMessage: '保存',
        applyMessage: '应用',
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {
                        ...
                        //***增加：连续序号***//
                        if(p.continuousrownumber){
                        	var r1 = parseInt((p.page - 1) * p.pageSize) + 1.0;
                            gridhtmlarr.push('>' + (parseInt(item['__index']) + r1) + '</div></td>');
                        }else{
